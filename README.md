...
- **`all`** — Before capture, deletes `last_alert_prices.json` for the active symbol (or `--last-alert-json` path) so the run does not inherit yesterday’s statuses / `trade_line` / tickets. Pass **`--no-clear-last-alert`** to keep the existing file and merge as before. OpenAI runs **2-batch chained by default**: batch 1 = TradingView structure (DXY + main H4/H1/M15/ICT/M5), batch 2 = GoCharting footprint + Schema A (chained via `previous_response_id`). Pass **`--no-all-two-phase`** to send the full payload in one call (legacy). Custom **`--prompt`** disables 2-batch automatically. On OpenAI success, writes **`last_response_id.txt`** and **`last_all_response_id.txt`** (same response id for intraday routing). On success, if the model output parses as JSON, **`data/{SYMBOL}/morning_full_analysis.json`** is written (exact Schema A object for intraday).
