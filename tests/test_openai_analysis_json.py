...
def test_select_zone_scalp_uses_lower_threshold() -> None:
    tl = "BUY LIMIT 1 | SL 0 | TP1 2 | Lot 0.01"
    assert select_zone_for_auto_mt5([PriceZoneEntry("scalp", 1.0, hop_luu=50, trade_line=tl)]) is None
    assert select_zone_for_auto_mt5([PriceZoneEntry("scalp", 1.0, hop_luu=51, trade_line=tl)]) is not None


def test_select_zone_for_label_scalp_threshold() -> None:
    tl = "BUY LIMIT 1 | SL 0 | TP1 2 | Lot 0.01"
    assert select_zone_for_auto_mt5_for_label(
        [PriceZoneEntry("scalp", 1.0, hop_luu=51, trade_line=tl)], "scalp"
    ) is not None
    assert select_zone_for_auto_mt5_for_label(
        [PriceZoneEntry("scalp", 1.0, hop_luu=50, trade_line=tl)], "scalp"
    ) is None
