...
def test_tp1_followup_skip_trade_management_still_cancels_pending_without_position(
    monkeypatch, tmp_path
) -> None:
    state_path = tmp_path / "zones_state.json"
    zone = Zone(
    ...
    monkeypatch.setattr(
        daemon,
        "_send_user_notice",
        lambda _settings, title, *_args, **_kwargs: notices.append(title),
    )
    monkeypatch.setattr(daemon, "load_mt5_accounts_for_cli", lambda *_args, **_kwargs: [])
    monkeypatch.setattr(
        daemon,
    ...
    daemon._tp1_followup_job(
        settings=SimpleNamespace(skip_trade_management=True),
        params=params,
        zone_id="z-scalp",
        p_last=4512.0,
    )
    ...
    assert updated.zones[0].mt5_ticket is None
    assert updated.zones[0].tp1_followup_done is True
    assert any("Chạm TP1 nhưng lệnh chưa khớp" in t for t in notices)
